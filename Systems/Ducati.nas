...
var forkcontrol = func{
	...
	if(cv == 0 and !du){
		if(apos == 49 and press){
			...
			setprop("/controls/Ducati/driver-looks-back",0);
			setprop("/controls/Ducati/driver-looks-back-right",0);
		}
	}
...
setlistener("/surface-positions/left-aileron-pos-norm", func{
	...
	var omm = getprop("/controls/Ducati/old-mens-mode") or 0;
	...
	var driverpos = getprop("/controls/Ducati/driver-up") or 0;
...
setlistener("/instrumentation/airspeed-indicator/indicated-speed-kt", func (speed){
	...
	if(getprop("/instrumentation/Ducati/speed-indicator/selection")){
		if(groundspeed > 0.1){
			setprop("/instrumentation/Ducati/speed-indicator/speed-meter", speed*1.15077945); # mph
		}else{
			setprop("/instrumentation/Ducati/speed-indicator/speed-meter", 0);
		}
	}else{
		if(groundspeed > 0.1){
			setprop("/instrumentation/Ducati/speed-indicator/speed-meter", speed*1.852); # km/h
		}else{
			setprop("/instrumentation/Ducati/speed-indicator/speed-meter", 0);
		}
	}
...
setlistener("/instrumentation/Ducati/speed-indicator/speed-limiter", func (sl){
...
setlistener("sim/model/start-idling", func()
 ...
 setlistener("/controls/gear/brake-parking", func(p)
  ...
  if (p.getValue() == 1)
   {
   		interpolate("/controls/Ducati/driver-up", 1, 0.8);
   }
  else
   {
   		interpolate("/controls/Ducati/driver-up", 0, 0.8);
   }
  }, 1, 0);
