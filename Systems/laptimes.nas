...
var racelap = props.globals.getNode("/Ducati/race-lap");
var thissector = props.globals.getNode("/Ducati/this-sector");
# only for mp result view
var sectortime = props.globals.getNode("/Ducati/this-sector-time");
var laptime = props.globals.getNode("/Ducati/this-lap-time");
var racetime = props.globals.getNode("/Ducati/this-race-time");
var laptimediff = props.globals.initNode("/Ducati/this-lap-time-diff",0,"DOUBLE");
var laptimediffmin = props.globals.initNode("/Ducati/this-lap-time-diff-m",0,"DOUBLE");
var laptimediffsec = props.globals.initNode("/Ducati/this-lap-time-diff-s",0,"DOUBLE");
var inrange = 0;
...
var total_result = func{
	var maxlaps = getprop("/Ducati/"~pa~"/race-laps") or 0;
	var tr = 0;
    for(v = 0; v <= maxlaps; v +=1) {
		var actl = getprop("/Ducati/"~pa~"/lap["~v~"]/actual-time") or 0;
		tr += actl;
	}
...
var show_lap_and_sector_time = func{
	var show_it = getprop("/Ducati/show-race-times") or 0;
	var show_l_or_f = getprop("/Ducati/switch-last-fastest") or 0;
	...
	foreach(var s; sectors) { 
		...
		var lsts = getprop("/Ducati/"~pa~"/sector["~ln~"]/start-time") or 0;
		var sts  = getprop("/Ducati/"~pa~"/sector["~n~"]/start-time") or 0;
		var lt  = getprop("/Ducati/"~pa~"/sector["~n~"]/last-time") or 0;
		var ft  = getprop("/Ducati/"~pa~"/sector["~n~"]/fastest-time") or 0;
		...
		if(thsec == n and racelap.getValue() > 0){
			...
			if(ct > 0){
				...
				setprop("Ducati/this-sector-time-sec", result[0]);
				setprop("Ducati/this-sector-time-min", result[1]);
			}
			if(ft > 0){
				var result = calc_time(ft);
				setprop("Ducati/fastest-sector-time-sec", result[0]);
				setprop("Ducati/fastest-sector-time-min", result[1]);				
			}
			# only for mp
	...
	var fl = getprop("/Ducati/"~pa~"/fastest-lap") or 0;
	var ll = getprop("/Ducati/last-lap-time") or 0;
	...
	if(fl > 0){
		...
		setprop("Ducati/fastest-lap-time-sec", result[0]);
		setprop("Ducati/fastest-lap-time-min", result[1]);
		setprop("Ducati/fastest-lap-time-hour", result[2]);
	}
	if(lapt > 0){
		var result = calc_time(lapt);
		setprop("Ducati/this-lap-time-sec", result[0]);
		setprop("Ducati/this-lap-time-min", result[1]);
		setprop("Ducati/this-lap-time-hour", result[2]);
	}
	# only for mp
...
var find_marker = func{
	...
	if(getprop("/Ducati/"~pa~"/sector["~p~"]/fastest-time") == 0){
	...
	if(wp_distance < 260){
		var courserl = getprop("/Ducati/"~pa~"/race-laps") or 0;
	...
	if(wp_distance < 50){
		setprop("/Ducati/"~pa~"/sector["~thissector.getValue()~"]/start-time", getprop("/sim/time/elapsed-sec"));
		#print ("Sie ueberfuhren den "~thissector.getValue()~". Wegpunkt "~sectors[thissector.getValue()][3]);
		setprop("/Ducati/sector",sectors[thissector.getValue()][3]);

		var fastesttime = getprop("/Ducati/"~pa~"/sector["~ln~"]/fastest-time") or 0;
		var lastsectorstarttime = getprop("/Ducati/"~pa~"/sector["~ln~"]/start-time") or 0;
		var lastsectorendtime = getprop("/Ducati/"~pa~"/sector["~thissector.getValue()~"]/start-time") or 0;
		...
		setprop("/Ducati/"~pa~"/sector["~ln~"]/last-time", lasttime);
		...
		if(lasttime > 0 and lasttime < fastesttime or fastesttime == 0) setprop("/Ducati/"~pa~"/sector["~ln~"]/fastest-time", lasttime);
		...
		if(thissector.getValue() == 0){
		  ...
		  foreach(var ls; sectors) { 
		  	totallapresult += getprop("/Ducati/"~pa~"/sector["~n~"]/last-time") or 0;
			n+=1;
		  }
		  setprop("/Ducati/last-lap-time", totallapresult);
		  var fastestlap = getprop("/Ducati/"~pa~"/fastest-lap") or 0;
		  
		  # if actual time is less than fastest laptime
		  ldiff = (racelap.getValue() > 0) ? fastestlap - totallapresult : 0;
		  ######
		  
		  if(totallapresult > 0 and totallapresult < fastestlap or fastestlap == 0) setprop("/Ducati/"~pa~"/fastest-lap", totallapresult);
		  setprop("/Ducati/"~pa~"/lap["~racelap.getValue()~"]/actual-time", totallapresult);
		...
		setprop("/Ducati/this-lap-time-diff-s", resultldiff[0]);
		setprop("/Ducati/this-lap-time-diff-m", resultldiff[1]);
...
var clear_race_datas = func{
	...
	if(pa != nil and inrange != nil and inrange == 1){
		...
		laptime.setValue(0);
		racetime.setValue(0);
		var act = props.globals.getNode("/Ducati/"~pa);
		
		setprop("/Ducati/last-lap-time",0);
