...
var speedlimiter = props.globals.getNode("/instrumentation/Ducati/speed-indicator/speed-limiter");
var speedlimstate = props.globals.getNode("/instrumentation/Ducati/speed-indicator/speed-limiter-switch");
var speed = 0;
var gspeed = 0;
var ascon = props.globals.initNode("/controls/Ducati/SCS/on-off",1,"BOOL");
...
var loop = func {
	...
	if(running.getValue() == 1){
		...
		speed = getprop("/instrumentation/Ducati/speed-indicator/speed-meter");
		...
		if(rpm.getValue() > maxrpm-2100 or speedlimstate.getBoolValue() == 1){
			setprop("/controls/Ducati/ctrl-light-overspeed", 1);
		}else{
			setprop("/controls/Ducati/ctrl-light-overspeed", 0);
		}		
	...
	if(comp_m < 0.05 and brake_ctrl_right > 0.5 and brake_ctrl_left > 0.5 and gspeed > 50){
		setprop("/controls/Ducati/ABS/ctrl-light", 1);
		setprop("/controls/Ducati/ctrl-light-overspeed", 1);
		setprop("/controls/Ducati/ABS/brake-right", brake_ctrl_right*0.34);
		setprop("/controls/Ducati/ABS/brake-left", brake_ctrl_left*0.34);		
	}else{
		setprop("/controls/Ducati/ABS/ctrl-light", 0);
		setprop("/controls/Ducati/ABS/brake-right", brake_ctrl_right);
		setprop("/controls/Ducati/ABS/brake-left", brake_ctrl_left);
	}
